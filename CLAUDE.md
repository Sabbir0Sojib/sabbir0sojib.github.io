...
- `404.html` (GitHub Pages serves it for any missing address): generated by `build-pages.py` with root-relative links (`absolutize()`), text editable in `site.json` (`notfound_title`, `notfound_text`). `site.js` loads content and icons from the site root (`ROOT`, a path such as `/`, never a full address, so pre-rendered HTML is identical locally and live), so pages work at any depth.
...
1. Page shells are generated by `python3 .impeccable/build-pages.py` (edit the generator, not the HTML). It also writes `404.html`, `sitemap.xml` (with every map and photo as an image entry) and `robots.txt`, the Person structured data from `content/profile.json`, and structured data for Maps (map collection), Gallery (image gallery) and Projects (code list). It also writes one page per map, `maps/<name>.html` (Map and breadcrumb structured data, the map as the link preview image); the name comes from the map title, by the same rule as `mapSlug()` in `site.js`, and pages for removed maps are deleted. Do not edit files in `maps/` by hand.
...
2. **Impeccable** (`impeccable`): reference guides in `.claude/skills/impeccable/reference/`, and the detector:
   ...
   Keep it at zero findings except the known false positives: `cramped-padding` on `.block`, `.split`, `.utility` and `.band` (the inner `.wrap` and page head carry the padding), `cream-palette` (the warm paper page is the owner's Overlake reference) and `tight-leading` on pre-rendered titles (a file-scan artifact). In cloud sessions running as root, point `IMPECCABLE_BROWSER` at a small script that runs `/opt/pw-browsers/chromium-1194/chrome-linux/chrome --no-sandbox "$@"`. The browser scan is the real check and should report zero (also at `820x1180` and `1024x768`): `impeccable detect --json --viewport 1280x800 http://127.0.0.1:4000/index.html ...` and again at `390x844`. Keep `DESIGN.md` in sync, since the detector checks colours, sizes and radii against it.
...
4. **Awesome Design** (`.claude/design-references/<brand>/DESIGN.md`, 74 brands): consult for patterns. Used so far: Overlake School (owner's reference, not in the folder: green bands, yellow action pill, utility bar), Apple (work on a quiet pedestal, full footer), Pinterest (filter chips; Gallery: the photo is the card, 8px gutters, chrome out of the photo's way), Airbnb (Gallery: one lead photo with smaller ones, as on a listing page), Wired (equal research rows).
